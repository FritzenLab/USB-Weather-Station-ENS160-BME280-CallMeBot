...
void loop() {
  ...
  if(millis() - oldtime >= 60000){ // 60 seconds
    ...
    if ((h == 0 || h == 6 || h == 12) && (m == 0 || m == 1)) {  // Send message at 0h, 6h, 12h and 18h, 0 minutes or 1 minute (only once though)
      if(hasnotpassedhere == 1){ // guarantees that the message is sent only once
        hasnotpassedhere == 0;
        sendMessage("Temperature= " + String(temp) + " oC" + "\n Humidity= " + String(hum) +"%"); // CallMeBot Whatsapp message
      }
      
    }else if((h == 0 || h == 6 || h == 12) && (m == 2 || m == 3)){ // on minute 2 or 3 it wil reset the "send message" counter
      hasnotpassedhere == 1;
    }
